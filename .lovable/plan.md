...
Core→brand import in `PublicChrome`, inquiry-form logic in the brand layer, oversized route/lib files, `text-white` / `bg-black` token violations, root `"Lovable App"` head defaults.
...
- Verification: build + typecheck, then load `/en` and `/de`, the listings index and the about page to confirm titles, descriptions, placeholder and portrait all render from settings.
